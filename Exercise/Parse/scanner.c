...
Token* getToken(void){
	...
	case 3:{
		...
		while ((currentChar != EOF) &&
			...
			if (count <= MAX_IDENT_LEN) str[count++] = (char)currentChar;
			readChar();
		}
	...
	case 16:
		readChar();
		if ((currentChar != EOF) && (charCodes[currentChar] == CHAR_EQ)) state = 17;
		else state = 18;
		return getToken();
	case 17:
		readChar();
	...
	case 20:
		readChar();
		if ((currentChar != EOF) && (charCodes[currentChar] == CHAR_EQ)) state = 21;
		else state = 22;
		return getToken();
	case 21:
		readChar();
	...
	case 27:
		...
		cn = colNo;
		readChar();
		if (ln != lineNo) return makeToken(SB_SEMICOLON, ln, cn);
		return makeToken(SB_SEMICOLON, lineNo, colNo - 1);
	case 28:
		readChar();
		if ((currentChar != EOF) && (charCodes[currentChar] == CHAR_EQ)) state = 29;
		else state = 30;
		return getToken();
	case 29:
		readChar();
