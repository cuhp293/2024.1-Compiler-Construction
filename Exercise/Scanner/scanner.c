...
{ 
  ...
  if (currentChar == EOF) {
    state = 41;
  } else if (charCodes[currentChar] == CHAR_TIMES) { // Opened caption detected *)
    state = 37;
  } else {
    state = 41;
  }
  ...
  while (currentChar != EOF) {
    if (charCodes[currentChar] == CHAR_TIMES) {
      readChar();
      if (charCodes[currentChar] == CHAR_RPAR) { // Closed caption detected *)
        state = 39;
        break;
      }
    } else {
  ...
  if (currentChar == EOF) {
    error(ERR_ENDOFCOMMENT, lineNo, colNo); // Error if EOF before end of comment
  }
  return getToken();
